...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def open_model(self):
        self.openfile_name_model, _ = QFileDialog.getOpenFileName(self.ui.pushButton_weights, '选择weights文件',
                                                             'weights/')
        print(self.openfile_name_model, type(self.openfile_name_model))
    ...
    def model_init(self):
        ...
        self.half = self.device.type != ''  # half precision only supported on CUDA
        ...
        if classify:
            ...
            modelc.load_state_dict(torch.load('../../../OneDrive/桌面/yolov5-pyqt5/weights/hole.pt', map_location=self.device)['model']).to(self.device).eval()
