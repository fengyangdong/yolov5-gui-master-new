        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # 把坐标输入的number中。
                    ...
                    if number__ != 0:
                        print('执行')
                        ui.label_word3.setText(ui.label_word2.text())
                        ui.label_word2.setText(ui.label_word1.text())
                        ui.label_word1.setText(number__)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/hole.pt', help='model path or triton URL')
...
if __name__ == '__main__':
    ...
    class Ui_MainWindow(QtWidgets.QWidget):
        ...
        def set_ui(self):
            ...
            self.button_open = QtWidgets.QPushButton('开始')  # 建立用于打开摄像头的按键
            self.button_close = QtWidgets.QPushButton('退出')  # 建立用于退出程序的按键
            self.label_show = QtWidgets.QLabel()
            self.label_show.setFixedSize(500,500)
            self.label_word1 = QtWidgets.QLabel()
            self.label_word2 = QtWidgets.QLabel()
            self.label_word3 = QtWidgets.QLabel()
            self.label_word1.setFixedSize(500, 50)
            self.label_word2.setFixedSize(500, 50)
            self.label_word3.setFixedSize(500, 50)
            ...
            '''data'''
            self.__layout_data_show.addWidget(self.label_show)
            self.__layout_data_show.addWidget(self.label_word1)
            self.__layout_data_show.addWidget(self.label_word2)
            self.__layout_data_show.addWidget(self.label_word3)
        ...
        def slot_init(self):
            ...
            self.button_close.clicked.connect(
                self.close)  # 若该按键被点击，则调用close()，注意这个close是父类QtWidgets.QWidget自带的，会关闭程序
            self.timer_camera.timeout.connect(self.show_camera)  # 若定时器结束，则调用show_camera()

        def button_open_clicked(self):
            if self.button_open.text() == "开始":
                self.label_word1.setText("开始执行")
